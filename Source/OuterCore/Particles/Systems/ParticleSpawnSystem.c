...
ECS_SYSTEM_DECLARE(ParticleSpawnSystem);
        /*if (positions[i].value.x < - positionBounds.x)
        {
            positions[i].value.x = - positionBounds.x;
        }
        else if (positions[i].value.x > positionBounds.x)
        {
            positions[i].value.x = positionBounds.x;
        }
        if (positions[i].value.y < - positionBounds.x)
        {
            positions[i].value.y = - positionBounds.x;
        }
        else if (positions[i].value.y > positionBounds.x)
        {
            positions[i].value.y = positionBounds.x;
        }*/
        /*if (velocitys[i].value.x < - velocityBounds.x)
        {
            velocitys[i].value.x = - velocityBounds.x;
        }
        else if (velocitys[i].value.x > velocityBounds.x)
        {
            velocitys[i].value.x = velocityBounds.x;
        }
        if (velocitys[i].value.y < - velocityBounds.x)
        {
            velocitys[i].value.y = - velocityBounds.x;
        }
        else if (velocitys[i].value.y > velocityBounds.x)
        {
            velocitys[i].value.y = velocityBounds.x;
        }
        if (velocitys[i].value.z < - velocityBounds.x)
        {
            velocitys[i].value.z = - velocityBounds.x;
        }
        else if (velocitys[i].value.z > velocityBounds.x)
        {
            velocitys[i].value.z = velocityBounds.x;
        }*/