...
{
    setup_window();
    int supports_compute2 = check_compute_shader_support_initial();
    if (supports_compute2 == 0) {
        printf("Failed to support Compute Shaders (initial).\n");
    } else {
        printf("Supports Compute Shaders (initial).\n");
    }
