...
void spawn_character_actions(ecs_world_t *world, spawned_character3D_data data) {
    ...
    // skill!
    if (meta_skill_punch) {
        actions->value[0] = spawn_user_skill(world, meta_skill_punch, data.e);
    }
    if (meta_skill_aura_death) {
        actions->value[1] = spawn_user_skill(world, meta_skill_aura_death, data.e);
    }
    if (meta_item_block_dark) {
        actions->value[2] = spawn_user_item(world, meta_item_block_dark, data.e);
    }
    if (meta_item_block_dungeon_core) {
        actions->value[3] = spawn_user_item(world, meta_item_block_dungeon_core, data.e);
    }
    if (meta_item_block_dark) {
        actions->value[4] = spawn_user_item(world, meta_item_block_dark, data.e);
