...
ecs_entity_t spawn_quad_count_label(ecs_world_t *world, const ecs_entity_t canvas) {
    ...
    const ecs_entity_t e = spawn_label_background(world, prefab_quad_count_label, parent, canvas, pixel_position, anchor, padding, "", font_size, zox_mesh_alignment_right, layer, int2_half(parent_pixel_size), parent_pixel_size);
    return e;
}
