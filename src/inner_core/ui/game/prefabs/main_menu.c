...
ecs_entity_t spawn_main_menu(ecs_world_t *world, const char *header_label, int2 position, float2 anchor, unsigned char is_close_button) {
    ...
    ecs_entity_t e = spawn_ui_list(world, prefab_main_menu, main_canvas, "zoxel", labels_count, labels_count, labels, events, position, anchor, is_close_button, 42, layer, 0);
    zox_name("main_menu")
    main_menu = e;
    return e;
}
