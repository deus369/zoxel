...
ecs_entity_t spawn_zigel(ecs_world_t *world, const ZigelSpawnData *data) {
    ...
    const int2 global_position = get_element_pixel_position_global(data->parent.position, data->parent.size, data->position, data->anchor);
    ...
    initialize_element(world, e, data->parent.e, data->canvas.e, data->position, data->size, texture_size, data->anchor, data->layer, position, global_position);
    return e;
}
