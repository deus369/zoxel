...
int spawn_zext_zigels(ecs_world_t *world, ecs_entity_t zext, Children *children, const ZextData *zextData, int font_size, unsigned char text_alignment, byte2 text_padding, unsigned char zext_layer, float2 parent_position, int2 parent_pixel_size) {
    ...
    if (children->length != zextData->length) {
        ...
        if (new_children_length > old_children_length) {
            ...
            for (int i = old_children_length; i < new_children_length; i++) {
                new_children[i] = spawn_zext_zigel(world, zext, zigel_layer, i, new_children_length, zextData->value[i], font_size, text_alignment, text_padding, parent_position, parent_pixel_size, canvas_size);
            }
    ...
    #ifdef zoxel_debug_zext_updates
        else zoxel_log("    - zext remained the same [%i]\n", zextData->length);
    #endif
    return 0;
}