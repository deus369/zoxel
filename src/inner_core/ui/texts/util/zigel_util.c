unsigned char get_zext_max_x(const unsigned char *data, const unsigned char length) {
    unsigned char x = 0;
...
unsigned char get_zext_max_y(const unsigned char *data, const unsigned char length) {
    unsigned char y = 1;
    for (unsigned char i = 0; i < length; i++) {
...
unsigned char get_zext_x(const unsigned char *data, const unsigned char length, const unsigned char data_index) {
    unsigned char x = 0;
...
unsigned char get_zext_y(const unsigned char *data, const unsigned char length, const unsigned char data_index) {
    unsigned char y = 0;
    for (unsigned char i = 0; i < data_index; i++) {
        if (data[i] == zox_char_newline && i != length - 1) y++;
    }
...
unsigned char calculate_total_zigels(const unsigned char *data, const unsigned char length) {
...
unsigned char calculate_zigel_index(const unsigned char *data, const unsigned char length, const unsigned char spawn_index) {
    unsigned char j = 0;
    for (unsigned char i = 0; i < length; i++) {
        if (data[i] != zox_char_newline) {
...
unsigned char calculate_zigel_data_index(const unsigned char *data, const unsigned char length, const unsigned char spawn_index) {
    unsigned char j = 0;
    for (unsigned char i = 0; i < length; i++) {
        if (data[i] != zox_char_newline) {
...
int2 calculate_zext_size(const unsigned char *data, const unsigned char length, const unsigned char font_size, const byte2 padding) {
    unsigned char x = get_zext_max_x(data, length);
    unsigned char y = get_zext_max_y(data, length);
    // zox_log("sizey: %i\n", y)
    return (int2) { font_size * x + padding.x * 2, font_size * y + padding.y * 2 };
}

int2 calculate_zigel_position(const unsigned char *data, const unsigned char length, const unsigned char data_index, const unsigned char font_size, const unsigned char text_alignment, const byte2 padding) {
    const unsigned char x = get_zext_x(data, length, data_index);
    const unsigned char y = get_zext_y(data, length, data_index);
    const int2 size = calculate_zext_size(data, length, font_size, padding);
...
void set_zigel_position(ecs_world_t *world, const ZextData *zextData, const ecs_entity_t e, const unsigned char data_index, const int font_size, const unsigned char text_alignment, const byte2 text_padding, float2 anchor, const unsigned char zigels_count, const int2 parent_position, const int2 parent_size, const int2 canvas_size) {
    const int2 pixel_position = calculate_zigel_position(zextData->value, zextData->length, data_index, font_size, text_alignment, text_padding);
