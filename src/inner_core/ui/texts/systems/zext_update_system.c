...
void ZextUpdateSystem(ecs_iter_t *it) {
    ...
    const CanvasPixelPosition *canvasPixelPositions = ecs_field(it, CanvasPixelPosition, 6);
    ...
    for (int i = 0; i < it->count; i++) {
        ...
        spawn_zext_zigels(world, e, canvas, children, zextData, zextSize->value, meshAlignment->value, zextPadding->value, layer2D->value, canvasPixelPosition->value, pixelSize->value, renderDisabled->value);
        zextDirty->value = 2;
