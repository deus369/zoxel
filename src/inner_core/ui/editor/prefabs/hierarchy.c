...
ecs_entity_t spawn_editor_hierarchy(ecs_world_t *world, const ecs_entity_t canvas) {
    ...
    const ecs_entity_t e = spawn_ui_list(world, prefab_hierarchy, canvas, header_label, labels_count, max_elements, NULL, NULL, position, anchor, is_close_button, font_size, ui_layer, 1);
    zox_name("hierarchy")
    hierarchy = e;
    return e;
}
