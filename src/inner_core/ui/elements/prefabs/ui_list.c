...
ecs_entity_t spawn_ui_list(ecs_world_t *world, ecs_entity_t prefab, ecs_entity_t canvas, const char *header_label, int elements_count, int max_elements, const text_group labels[], const ClickEvent events[], int2 position, float2 anchor, unsigned char is_close_button, unsigned char font_size, unsigned char layer) {
    ...
    int2 window_size = { (font_size) * max_characters + button_padding.x * 2 + list_margins.x * 2, (font_size + button_padding.y * 2) * max_elements + button_inner_margins * (max_elements - 1) + list_margins.y * 2 };
    ...
    for (int i = 0; i < elements_count; i++) {
        int2 label_position = (int2) { 0, (int) (window_size.y / 2) - (i + 0.5f) * (font_size + button_padding.y * 2) - list_margins.y - i * button_inner_margins };
        if (is_scrollbar) label_position.x -= (scrollbar_width + scrollbar_margins * 2) / 2;
        ecs_entity_t button = spawn_button(world, e, label_position, button_padding, float2_half, labels[i].text, font_size, button_layer, position2D, window_size, canvas_size, 0);
