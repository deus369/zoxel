...
ecs_entity_t spawn_elementbar2D(ecs_world_t *world, const ecs_entity_t prefab, const ecs_entity_t ui_holder, const ecs_entity_t canvas, const ecs_entity_t parent, int2 pixel_position, const int2 pixel_size, const byte2 padding, const float2 anchor, const unsigned char layer, const int2 parent_pixel_position_global, const int2 parent_pixel_size, const int2 canvas_size, const unsigned char render_disabled) {
    ...
    unsigned char font_size = pixel_size.y - padding.y * 2;
    const int zext_width = pixel_size.x - padding.x * 2;
    ...
    const ecs_entity_t front_bar = spawn_elementbar2D_front(world, canvas, e, pixel_position_global, pixel_size, canvas_size, front_bar_layer, render_disabled);
    ZextSpawnData zextSpawnData = {
        .canvas = { .e = canvas, .size = canvas_size },
        .parent = { .e = e, .position = pixel_position_global, .size = pixel_size },
        .element = { .layer = zext_layer, .position = int2_zero, .anchor = float2_half, .render_disabled = render_disabled, .size = zext_size },
        .prefab = prefab_zext,
        .font_size = font_size,
        // .padding = padding,
