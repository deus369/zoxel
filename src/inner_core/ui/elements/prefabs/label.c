...
ecs_entity_t spawn_label(ecs_world_t *world, ecs_entity_t prefab, ecs_entity_t parent, int2 position, float2 anchor,
    const char* text, int font_size, unsigned char alignment, unsigned char layer, float2 parent_position2D,
    int2 parent_pixel_size) {
    ecs_entity_t e = spawn_zext(world, prefab, parent, position, anchor, text, font_size, alignment, layer, parent_position2D, parent_pixel_size);
