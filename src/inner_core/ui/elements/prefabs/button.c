...
ecs_entity_t spawn_button(ecs_world_t *world, ecs_entity_t parent, ecs_entity_t canvas, int2 pixel_position, int2 padding, float2 anchor, const char* text, int font_size, unsigned char layer, int2 parent_pixel_position_global, int2 parent_pixel_size, int2 canvas_size, unsigned char render_disabled) {
    unsigned char zext_layer = layer + 1;
    int2 zext_size = (int2) { font_size * strlen(text), font_size };
    int2 size = (int2) { zext_size.x + padding.x * 2, zext_size.y + padding.y * 2 };
    ...
    ecs_entity_t zext = spawn_zext(world, zext_prefab, e, canvas, int2_zero, float2_half, int2_to_byte2(padding), text, font_size, 0, zext_layer, pixel_position_global, zext_size, render_disabled);
...
ecs_entity_t spawn_button_on_canvas(ecs_world_t *world, ecs_entity_t canvas, int2 pixel_position, byte2 padding, const color color, const char* text, int font_size, float2 anchor, const ClickEvent event) {
    ...
    Children *children = zox_get_mut(e, Children)
    resize_memory_component(Children, children, ecs_entity_t, 1)
    children->value[0] = spawn_zext(world, zext_prefab, e, canvas, (int2) { 0, 0 }, (float2) { 0.5f, 0.5f }, padding, text, font_size, 0, (layer + 1), pixel_position_global, zext_size, 0);
    zox_modified(e, Children)
#ifdef zoxel_debug_spawns
