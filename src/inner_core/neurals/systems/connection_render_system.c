    ...
    for (int i = 0; i < it->count; i++) {
        ...
        if (neuron_a != 0 && neuron_b != 0) {
            ...
            if (signal->value == 1) {
                ...
                signal_position = float2_add(signal_position, float2_multiply_float(float2_subtract(neuron_b_position, neuron_a_position), signalStrength->value));
                spawn_line2D_square(world, signal_position, // (float2) { neuron_a_position.x + (x_position2 - x_position) * signalStrength->value, y_position },
