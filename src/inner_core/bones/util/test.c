void make_test_skeleton(ecs_world_t *world, const ecs_entity_t e) {
    ...
    // add bone one
    const ecs_entity_t bone = spawn_bone(world, prefab_bone, float3_zero);
    ...
    const ecs_entity_t bone_child = spawn_bone(world, prefab_bone, float3_zero);
    zox_set(bone_child, ParentLink, { bone })
    zox_set(bone_child, LocalPosition3D, { test_bone_position})
