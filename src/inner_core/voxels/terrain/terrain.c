...
int get_chunk_index_2(int i, int j, int k, int rows)
{
    ...
    j += rows;
    k += terrain_vertical;
    return i * (rows + rows + 1) + j + k * (rows + rows + 1) * (rows + rows + 1);
}
...
void TerrainImport(ecs_world_t *world)
{
    ...
    int chunks_total_length = (terrain_vertical + terrain_vertical + 1) * (terrain_rows + terrain_rows + 1) * (terrain_rows + terrain_rows + 1);
    ...
    for (int i = -terrain_rows; i <= terrain_rows; i++)
    {
        ...
        {
            for (int k = -terrain_vertical; k <= terrain_vertical; k++)
            {
                ...
                chunks[get_chunk_index_2(i, j, k, terrain_rows)] = spawn_terrain_chunk_octree(world,
                    ...
                    (int3) { i, k, j },
                    (float3) { i * chunk_real_size, k * chunk_real_size, j * chunk_real_size },
                    0.5f);
                #else
        ...
        for (int j = -terrain_rows; j <= terrain_rows; j++)
        {
            for (int k = -terrain_vertical; k <= terrain_vertical; k++)
            {
                set_chunk_neighbors(world,
                    ...
                    i == terrain_rows ? 0 : chunks[get_chunk_index_2(i + 1, j, k, terrain_rows)],
                    j == -terrain_rows ? 0 : chunks[get_chunk_index_2(i, j - 1, k, terrain_rows)],
                    j == terrain_rows ? 0 : chunks[get_chunk_index_2(i, j + 1, k, terrain_rows)]);
            }
        }
    }
