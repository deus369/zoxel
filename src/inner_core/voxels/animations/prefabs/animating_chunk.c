...
ecs_entity_t spawn_prefab_animating_chunk(ecs_world_t *world) {
    ...
    const ecs_entity_t e = ecs_new_w_pair(world, EcsIsA, prefab_noise_chunk);
...
ecs_entity_t spawn_animating_chunk(ecs_world_t *world, const float3 position, const float scale, const unsigned char division) {
...
void test_animating_chunks(ecs_world_t *world, const float3 position) {
