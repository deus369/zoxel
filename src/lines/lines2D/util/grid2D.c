...
ecs_entity_t spawn_grid2D(ecs_world_t *world) {
    ...
    for (position.x = -size; position.x <= size; position.x += grid_size) {
        for (position.y = -size; position.y <= size; position.y += grid_size) {
            if (position.x != size) {
                add_to_Children(children, spawn_line2D_colored(world, position, (float2) { position.x + grid_size, position.y }, thickness, 0, grid_color));
            }
            if (position.y != size) {
                add_to_Children(children, spawn_line2D_colored(world, position, (float2) { position.x, position.y + grid_size }, thickness, 0, grid_color));
            }
        }
    }
    world_grid2D = e;
