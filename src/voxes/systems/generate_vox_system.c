...
void GenerateVoxSystem(ecs_iter_t *it) {
    ...
    for (int i = 0; i < it->count; i++) {
        ...
        if (zox_has(e, VoxRubble)) {
            ...
            vox_rubble(voxelNode, node_depth, size, voxel_range, rubble_count, rubble_height);
            ...
            voronoi3D(voxelNode, node_depth, size, voxel_range_2, black_voxel_3);
            noise_vox(voxelNode, node_depth, size, voxel_range_2, black_voxel_3);  // avoids black ones
            ...
            for (voxel_position.x = 0; voxel_position.x < size.x; voxel_position.x++) {
                for (voxel_position.z = 0; voxel_position.z < size.z; voxel_position.z++) {
                    for (voxel_position.y = 1; voxel_position.y < size.y - 1; voxel_position.y++) {
                        ...
                        const byte voxel = get_octree_voxel(voxelNode, &temp_position_1, node_depth);
                        byte3 temp_position_up = byte3_up(voxel_position);
                        const byte voxel_up = get_octree_voxel(voxelNode, &temp_position_up, node_depth);
                        ...
                        const byte voxel_down = get_octree_voxel(voxelNode, &temp_position_down, node_depth);
                        ...
                        if (voxel_position.x != size.x - 1) {
                            ...
                            const byte voxel_right = get_octree_voxel(voxelNode, &temp_position_right, node_depth);
                        ...
                        if (voxel_position.x != 0) {
                            ...
                            const byte voxel_left  = get_octree_voxel(voxelNode, &temp_position_left, node_depth);
                        ...
                        if (voxel_position.z != size.z - 1) {
                            ...
                            const byte voxel_front = get_octree_voxel(voxelNode, &temp_position_front, node_depth);
                        ...
                        if (voxel_position.z != 0) {
                            ...
                            const byte voxel_back  = get_octree_voxel(voxelNode, &temp_position_back, node_depth);
            ...
            add_to_ColorRGBs(colorRGBs, dirt_dark_voxel);
            byte black_voxel_3 = colorRGBs->length;
            noise_vox(voxelNode, node_depth, size, voxel_range, black_voxel_3);
        ...
        if (is_generate_vox_outlines) {
            vox_outlines(voxelNode, node_depth, size, black_voxel);
        }
