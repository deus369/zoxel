...
extern ecs_entity_t spawn_character3D_npc(ecs_world_t *world, const ecs_entity_t vox, const ecs_entity_t chunk, const float3 position, const float4 rotation, const byte character_lod, const byte render_disabled);
...
void Characters3DSpawnSystem(ecs_iter_t *it) {
    ...
    for (int i = 0; i < it->count; i++) {
        ...
        for (byte j = 0; j < characters_per_chunk_count; j++) {
            ...
            if (vox) {
               const ecs_entity_t character = spawn_character3D_npc(world, vox, e, position, rotation, vox_lod, renderDisabled->value);
