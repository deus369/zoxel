ecs_entity_t spawn_player_camera(ecs_world_t *world, const ecs_entity_t player, const unsigned char index, const float3 camera_position, const float4 camera_rotation, const int2 viewport_position, const int2 viewport_size, const float4 screen_to_canvas) {
    ...
    const ecs_entity_t e = spawn_base_camera(world, prefab_camera, camera_position, camera_rotation, fov, viewport_position, viewport_size, screen_to_canvas);
    zox_add_tag(e, Camera3D)
    const ecs_entity_t e2 = spawn_camera_ui(world, prefab_camera, viewport_position, viewport_size);
...
ecs_entity_t spawn_default_ui(ecs_world_t *world, const ecs_entity_t camera, const int2 dimensions, const float4 screen_to_canvas) {
    const ecs_entity_t canvas = spawn_canvas(world, prefab_canvas, camera, dimensions, screen_to_canvas);
