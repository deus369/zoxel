const char *menu_options_header_label = "Options";
...
ecs_entity_t spawn_menu_options(ecs_world_t *world, const ecs_entity_t player, const ecs_entity_t canvas, const int2 position, const float2 anchor) {
    ...
    const byte layer = 1;
    const ecs_entity_t e = spawn_ui_list(world, prefab_menu_options, canvas, menu_options_header_label, labels_count, labels_count, labels, events, position, anchor, is_close_button, header_font_size, font_size, layer, 0, player);
