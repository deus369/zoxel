...
void set_octree_voxel_final(VoxelNode *node, byte3 *position, const byte2 *set_octree_data, byte depth) {
    if (depth == set_octree_data->y || !has_children_VoxelNode(node)) {
        node->value = set_octree_data->x;
        return;
    }
    depth++;
    byte dividor = powers_of_two_byte[set_octree_data->y - depth];   // starts at chunk_length, ends at 1
    ...
    VoxelNode* kids = get_children_VoxelNode(node);
    set_octree_voxel_final(&kids[i], position, set_octree_data, depth);
}