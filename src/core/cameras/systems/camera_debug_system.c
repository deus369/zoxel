...
void CameraDebugSystem(ecs_iter_t *it) {
    ...
    for (int i = 0; i < it->count; i++) {
        ...
        for (int j = 0; j < 6; j++) {
            ...
            render_line3D(float3_zero, float3_multiply_float(planer.normal, planer.distance), (color_rgb) { 255, 255, 0 });
        }
        // when this works, disable renders based on frustum checks
        unsigned char is_seen = is_in_frustum(cameraPlanes->value, test_bounds2);
        color_rgb cube_color = miss_color;
        if (is_seen) cube_color = hit_color;
        spawn_cube_lines(world, test_bounds.center, test_bounds.extents, 16, spawn_life_time, cube_color);
