...
void generate_font_texture(color* data, const int2 size, const FontData *font_data, const color line_color,  const color fill_color, const byte is_shapes, byte fill_thickness, byte outline_thickness) {
    ...
    if (is_shapes) {
        ...
        fill_thickness++;
        generate_splotches_lines(data, size, font_data, line_color, fill_thickness);
    }
}